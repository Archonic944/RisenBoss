...
public enum RisenAbility implements Listener {
    ...
    GROUND_SLAM("Ground Slam", "This ability, when used, first launches you high in the air, the shoots you back on the ground, sending out a shockwave damaging anyone caught in the impact!", Material.GRASS, (byte) -1, 20, false, true, "tank", "1.4x damage"){
        protected boolean activationInternal(boolean multiply, RisenBoss boss){
            ...
            new BukkitRunnable(){ //down velocity triggers when player starts falling (or cancels after 1k ticks)
    ...
    REJUVENATE("Rejuvenate", "Hold down this ability to hone your focus in such a way that, if you are left undisturbed for 8 seconds, you completely regain all of your health.", Material.RED_ROSE, (byte) -1, 45, true, true, "tank", "-1s rejuvenation time"){
    ...
    HOT_TUB("Hot Tub", "Creates a relaxing lava pool under you that heals 1 health instead of damaging you.", Material.LAVA_BUCKET, (byte) -1, 40, true, true, "corrupter", "1.4x heal amount"){
    ...
    SPELL_BOMB("Spell Bomb",  "This ability shoots a flying potion out in the direction you're looking in, which, after a small duration, explodes, coating a 5-block radius in a dangerous, fast damaging cloud of... something.", Material.POTION, (byte) 16386, 20, false, true, "wizard", "1.2x explosion rate"){
        protected boolean activationInternal(boolean multiply, RisenBoss boss) {
    ...
    /*Broken! May or may not fix later. But for now, it's probably going unused.*/DUMMY("Dummy!", "Trick your opponents with this ability by placing a dummy version of yourself, and turning you invisible for 2 seconds!", Material.LEATHER_CHESTPLATE, (byte) -1, 27, true, false, "wizard", "1"){
    ...
    RisenAbility(String name, String description, Material icon, byte dataValue, int cooldownSecs, boolean registerEvents, boolean include, String multiClass, String amplifier){
        ...
        lore.add("");
        lore.add(ChatColor.YELLOW + "With " + StringUtil.capitalizeFirst(multiClass) + " class: " + ChatColor.GREEN + amplifier);
